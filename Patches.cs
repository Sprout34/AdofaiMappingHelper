...
namespace MappingHelper
{
    internal static class Patches
    {
        ...
        [HarmonyPatch(typeof(ADOFAI.LevelEditor.Controls.PropertyControl_Note), "Setup")]
        public static class NotePatch
        {
            public static void Postfix(PropertyControl_Note __instance, bool addListener)
            {
                if (__instance.propertyInfo.noteKey.Contains("MappingHelperSettings.DynamicDeco_note"))
                {
                    if (Main.Localizations.Get("mh.dynamicDeco_note", out string value, null))
                    {
                        __instance.noteText.text = value;
                    }
                }
            }
        }
        ...
        class PropertyInfoCtorPatch
        {
            ...
            {
                object fileTypeObj;
                if (dict.TryGetValue("fileType", out fileTypeObj))
                {
        ...
        internal static class BrowseFilePatch
        {
            public static bool Prefix(PropertyControl_File __instance)
            {
                ...
                if (fileType == (ADOFAI.FileType)FileTypeExtension.Directory)
                {
                    ...
                    //string[] paths = StandaloneFileBrowser.OpenFolderPanel(Main.Localizations.GetValue("mh.selectImagesDirectory"), Persistence.GetLastUsedFolder(), false);
                    ...
                    if (!Directory.Exists(targetDir))
                    {
                        ...
                        {
                            Main.Logger.Log("源文件夹不存在 / Source directory is not exist");
                            return false;
                        }
